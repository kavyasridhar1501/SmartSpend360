  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <AreaChart data={withMA} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
        ...
        />
        <Area type="monotone" dataKey="amount" stroke="#14b8a6" fill="url(#spendGradient)"
      ...
      {stages.map((stage: { status: string }, i: number) => (
        <div key={i} className="flex items-center gap-2">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-xs text-slate-500 whitespace-nowrap">{i + 1}</span>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Financial analytics at a glance</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-slate-800 border border-slate-700 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-slate-200 mb-4">
            Daily Spend — 90 Days
            <span className="ml-2 text-xs font-normal text-slate-500">with 7-day moving average</span>
          </h3>
          <SpendChart />
        </div>
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-slate-200 mb-4">Category Breakdown</h3>
          <CategoryDonut />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-200">Recent Anomalies</h3>
          ...
          <div className="space-y-3">
            {anomalies?.anomalies?.slice(0, 3).map((a: AnomalyRecord) => (
              <div key={a.alert_id} className="flex items-start justify-between p-3 bg-slate-700/30 rounded-lg">
                ...
                {a.daily_total && (
                  <span className="text-sm font-mono text-red-400">{formatCurrency(a.daily_total)}</span>
                )}
              </div>
            ))}
            {!anomalies?.anomalies?.length && (
              <p className="text-sm text-slate-500 text-center py-4">No anomalies detected</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-slate-200 mb-4">Week-over-Week Spend</h3>
            <WeeklyBarChart />
          </div>
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
            ...
            <PipelineWidget />
            <p className="text-xs text-slate-500 mt-3">7 stages — hover for details on Pipeline page</p>
          </div>
        </div>
      </div>
    </div>
